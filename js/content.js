...
window.SITE = {
  ...
  featured: [
    ...
    },
    {
      ...
      tagline: 'An AI-powered legal intelligence platform built around a graph of legal doctrine.',
      tags: ['Node.js', 'React', 'Supabase Auth'],
      metric: { value: '41k', label: 'lines of JS, TS and SQL I committed' },
      cardProblem: 'Winning a case means understanding how legal arguments connect, fork and defeat each other.',
      cardStack: 'Node.js, React, TypeScript, PostgreSQL, Supabase Auth, LLM extraction',
      cardOutcome: 'Live at praecept.ai. I built the production auth system and the LLM data pipeline.',
      problem: 'Legal research tools return documents. Lawyers need to see how doctrines connect, where arguments fork and what defeats them over time. Praecept models that as a typed doctrine graph that lawyers can query for argument paths, case citations and litigation strategy.',
      approach: 'I worked across the stack as a software engineer on a small team. I built a Node.js CLI that orchestrates a multi-stage data pipeline, combining LLM extraction, the CourtListener API and asynchronous task routing into a structured review workflow for the legal annotators. I also migrated authentication from custom JWTs to Supabase Auth, proxied through the backend so sessions live in HttpOnly cookies.',
      stack: 'Node.js and Express, React with TypeScript and Vite, PostgreSQL, Supabase Auth with Google OAuth (PKCE) and magic links, JWKS token verification, token revocation, Cloudflare-aware rate limiting, OpenAI structured outputs, CourtListener API.',
      outcome: 'A live product at praecept.ai. Sessions are HttpOnly-cookie based instead of browser-stored tokens, and the annotation pipeline feeds a review workflow used by the team\'s legal reviewers. The source is private.',
